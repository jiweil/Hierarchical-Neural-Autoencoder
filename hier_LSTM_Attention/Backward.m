    ...
    for sen_tt=T:-1:1
        for ll=parameter.layer_num:-1:1
            if ll==parameter.layer_num
                dh{ll}=dh{ll}+grad.source_sen(:,N*(sen_tt-1)+1:N*sen_tt);
            end
            if sen_tt==1
                c_t_1 =zeroState;
            else
...
function[grad]=Backward_Target(current_batch,result,grad,parameter)
    ...
    grad.Attention_U=zeroMatrix(size(parameter.Attention_U));
    grad.Attention_W=zeroMatrix(size(parameter.Attention_W));
    grad.vect=zeroMatrix(size(parameter.vect));

    N_target_sen=size(current_batch.target_sen_matrix,1);
    ...
    L=[];
    N_source=size(current_batch.source_sen_matrix,1);
    T_source=size(current_batch.source_sen_matrix,2);
    grad.source_sen=zeroMatrix([parameter.hidden,N_source*T_source]);
    ...
    for sen_tt=T_target_sen-1:-1:0
        ...
        for ll=parameter.layer_num:-1:1
            ...
            if ll~=1
                ...
                V=zeroMatrix([parameter.hidden,N_target_sen]);
                for i=1:T_source
                    V=V+d_input(parameter.hidden+1:2*parameter.hidden,N_source*(i-1)+1:N_source*i);
                end
                if sen_tt~=1
                    grad.target_sen_h{parameter.layer_num,sen_tt-1}=grad.target_sen_h{parameter.layer_num,sen_tt-1}+V;
                else
                    grad.source_h{parameter.layer_num,1}=grad.source_h{parameter.layer_num,1}+V;
                end
                clear V;
                clear d_input;
            end
    ...
    clear result.attention_input;
    clear result.attention_vector;
    clear result.dA;
    clear result.scores;
    clear result.sum_vector;

    for ll=1:parameter.layer_num
        grad.Word_T{ll}=grad.Word_T{ll}/num_sentence;
        grad.Sen_T{ll}=grad.Sen_T{ll}/num_sentence;
    end
    grad.Attention_U=grad.Attention_U/num_sentence;
    grad.Attention_W=grad.Attention_W/num_sentence;
